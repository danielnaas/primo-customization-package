...
// trying special validation here 
  function isSpecial(scope) {
    var validation = false;
    var result = getResult(scope);
//
//    if(result.delivery.bestlocation.libraryCode != "undefined") {
      if(result.delivery.displayLocation) {
      console.log("LIBRARYCODE:", result.delivery.bestlocation.libraryCode);
        if(result.delivery.bestlocation.libraryCode === "special") {
          //console.log("Here's what is in collection:", result);
          //var loc1 = result.pnx.facets.collection[0].trim().toLowerCase();
          console.log("yeah this thing's at spesh");
          validation = true;
          //if(loc1 === "special") {
          //  ;
          //}
...
//  end special validation 
  ...
  function isArticle(scope) {
    var validation = false;
    var result = getResult(scope);
    console.log("ALMA CHECK: ", result.delivery.displayedAvailability);
    console.log("ALMA CHECK2: ", result.delivery.availability[0]);
...
// get collection (special filter)
  function getCollection(scope) {
    var facetColl = "";
    var result = getResult(scope);

    if(result && result.pnx) {
      if(result.pnx.facets && result.pnx.facets.collection) {
        if(result.pnx.facets.collection[0]) {
          facetColl = result.pnx.facets.collection[0].trim();
        }
      }
    }

    return encodeURIComponent(facetColl);
  };

// end get collection
// get creator
  function getCreator(scope) {
    var creator = "";
    var result = getResult(scope);

    if(result && result.pnx) {
      if(result.pnx.display && result.pnx.display.creator) {
        if(result.pnx.display.creator[0]) {
          creator = result.pnx.display.creator[0].trim();
        }
      }
    }

    return encodeURIComponent(creator);
  };


// end get creator
// get linktouc
  function getLinktouc(scope) {
    var creator = "";
    var result = getResult(scope);

    if(result && result.pnx) {
      if(result.pnx.links && result.pnx.links.linktouc) {
        if(result.pnx.links.linktouc[0]) {
          creator = result.pnx.links.linktouc[0].trim();
        }
      }
    }
 ...
 // end get linktouc 
  ...
  function shouldEnhance(scope) {
    ...
    console.log("!isFiltered", !isFiltered(scope));
    if(!isFiltered(scope)) {
      console.log("isJournal(scope)", isJournal(scope));
      console.log("getIssn(scope)", getIssn(scope));
      if(isJournal(scope) && getIssn(scope)) {
        validation = true;
      }

      console.log("isArticle(scope)", isArticle(scope));
      console.log("getDoi(scope)", getDoi(scope));
      console.log("getCollection(scope)", getCollection(scope)); // grabbing collection
      console.log("getCreator(scope)", getCreator(scope)); // grabbing creator
      console.log("isSpecial(scope)", isSpecial(scope)); // grabbing isspecial      
      console.log("getLinktouc(scope)", getLinktouc(scope)); // grabbing linktouc
  ...
  function searchResult($scope) {
    ...
    if(!shouldEnhance(scope)) {
      console.log("shouldEnhance", false);
      console.log("---");

      // AEON non browzine stuff
          // var template = browzineWebLinkTemplate(scope, browzineWebLink);
          var element = getElement(scope);
          if(isSpecial(scope)) {
            // AEON collecting URL bits
            var result = getResult(scope);
            var urlTitle = "&ItemTitle=" + encodeURIComponent(result.pnx.addata.btitle[0]);
            var urlCreator = "&ItemAuthor=" + encodeURIComponent(result.pnx.sort.author[0]);;
            // more complex url pieces
            //var urlCallNo = "&CallNumber=" + result.delivery.holding[0].callNumber;
            var urlCallNo = "&CallNumber=" + encodeURIComponent(result.delivery.holding[0].callNumber);
            var urlSubLoc = "&Location=" + encodeURIComponent(result.delivery.bestlocation.subLocation);
            console.log("This item is at:",urlSubLoc); 
            var urlDate = "&ItemDate=" + encodeURIComponent(result.pnx.display.creationdate);
            var urlSource = "&ItemInfo2=" + encodeURIComponent("ttps://saalck-uky.primo.exlibrisgroup.com/permalink/01SAA_UKY/1p8rpi4/" + result.pnx.control.recordid[0]);
            var urlCombined = urlTitle + urlCreator + urlCallNo + urlSubLoc + urlDate + urlSource;
            // end more complex url pieces
            // get rid of junk chars
            var urlCombined = urlCombined.replace("'", "");
            // end collecting URL bits
          var aeonTemplate = "<div class='aeon' style='line-height: 1.4em;'>" +
                      "<a class='aeon-link' href='https://requests-libraries.uky.edu/logon?Action=10&Form=30&OpenURL?" + urlCombined + "' target='_blank'><div class='aeon' style='line-height: 1.4em;'><img class='aeonicon' src='https://saalck-uky.primo.exlibrisgroup.com/discovery/custom/01SAA_UKY-UKY/img/UKAeonfavicon2.png'>Submit Request to Special Collections Research Center</a><md-icon md-svg-icon='primo-ui:open-in-new' class='md-primoExplore-theme' aria-hidden='true' style='height: 15px; width: 15px; min-height: 15px; min-width: 15px; margin-top: -2px;'>" +
                   "</div>";
          
          } else {

          }
          element.append(aeonTemplate);
      // AEON non browzine stuff
      return;
    }

// adjust for Special Collections journals
    if(shouldEnhance(scope) && isSpecial(scope)) {
      console.log("shouldEnhance", true);
      console.log("---");
      ...
      // AEON non browzine stuff
          ...
          if(isSpecial(scope)) {
            ...
            // more complex url pieces
            //var urlCallNo = "&CallNumber=" + result.delivery.holding[0].callNumber;
            var urlCallNo = "&CallNumber=" + encodeURIComponent("CALLNUMBER");
            var urlSubLoc = "&Location=" + encodeURIComponent(result.delivery.bestlocation.subLocation);
            console.log("This item is at:",urlSubLoc); 
            var urlDate = "&ItemDate=" + encodeURIComponent(result.pnx.display.creationdate);
            var urlSource = "&ItemInfo2=" + encodeURIComponent("https://saalck-uky.primo.exlibrisgroup.com/permalink/01SAA_UKY/1p8rpi4/" + result.pnx.control.recordid[0]);
            var urlCombined = urlTitle + urlCreator + urlCallNo + urlSubLoc + urlDate + urlSource;
            // get rid of junk chars
            var urlCombined = urlCombined.replace("'", "");
            // end more complex url pieces
          var aeonTemplate = "<div class='aeon' style='line-height: 1.4em;'>" +
                      "<a class='aeon-link' href='https://requests-libraries.uky.edu/logon?Action=10&Form=30&OpenURL?" + urlCombined + "' target='_blank'><div class='aeon' style='line-height: 1.4em;'><img class='aeonicon' src='https://saalck-uky.primo.exlibrisgroup.com/discovery/custom/01SAA_UKY-UKY/img/UKAeonfavicon2.png'>Submit Request to Special Collections Research Center</a><a class='aeonhelp' href='https://saa-primo.hosted.exlibrisgroup.com/primo-explore/search?vid=UKY&lang=en_US&tab=default_tab&sortby=rank' style='font-size: .8em; margin-left: 5.25px; font-style: italic;' >(what's this?)</a><md-icon md-svg-icon='primo-ui:open-in-new' class='md-primoExplore-theme' aria-hidden='true' style='height: 15px; width: 15px; min-height: 15px; min-width: 15px; margin-top: -2px;'>" +
                   "</div>";
...
// end adjust for Special Collections journals
